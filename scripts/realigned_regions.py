...
def process_bam_files(before_bam, after_bam, output_bed):
    ...
    for read in after:
        key = (read.query_name, read.flag)
        current_position = (read.reference_name, read.reference_start, read.reference_end)
        ...
        if key in before_dict:
            ...
            if current_position[0] != previous_positions[0] or abs(current_position[1] - previous_positions[1]) > 500 or \
                    abs(current_position[2] - previous_positions[2]) > 500:
    ...
    for (query_name, flag), positions in before_dict.items():
        for ref_name, ref_start, ref_end in positions:
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python realigned_regions.py <before.bam> <after.bam> <output.bed>")
        sys.exit(1)

    before_bam = sys.argv[1]
    after_bam = sys.argv[2]
    output_bed = sys.argv[3]

    process_bam_files(before_bam, after_bam, output_bed)